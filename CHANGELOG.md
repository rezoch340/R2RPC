...
### 管理前端
- 所有数据表格统一通过公共 `useTableQuery` 每 15 秒自动刷新；返回已访问页面时立即后台获取
  最新数据，不再受短时缓存阻止。
- 全部筛选重置按钮在清空条件和回到第一页后强制刷新对应表格；分页只在真正
  切页时锁定，后台轮询期间保留已有数据和可操作控件。
- Playwright 新增切回页面、重置、定时刷新和非阻塞表格回归，当前基线为
  **13 passed**。

### 设备调度
- 设备上报的 `maxInFlight` 有效整数范围改为 `1～1024`，服务端不再把低并发设备强制抬高到
  `256`；缺失、非法或非正数使用保守默认值 `16`，仅对超过 `1024` 的值执行上限保护。
- JavaScript 与 Android SDK 默认主动上报 `16`，允许设备配置 `1～1024` 的实际容量，
  并在 SDK 初始化时拒绝非整数或超出范围的配置。
- 完整黑盒改用上报容量为 `4` 的设备验证真实 HTTP→WebSocket 占槽、跳过饱和设备、`429`
  拒绝和超时释放；新增归一化单元测试覆盖低值、默认值、非法值和服务端上限。

## [0.1.3] - 2026-07-28

### 管理前端
- 侧栏导航不再等待目标页业务接口，也不再在菜单项显示阻塞式进度；点击后立即切换页面，首次
  无缓存时只在目标页内容区显示稳定加载状态，已有缓存时继续复用页面数据。
- 浏览器黑盒新增慢接口导航回归，验证路由和标题先切换、旧页面立即卸载，接口完成后局部加载
  状态原位消失。
...
- 新增 1Panel 无限资源 Compose 生成脚本，完整保留 9 个服务和 PID 上限，同时移除全部 CPU、
  内存限制；部署与更新文档补齐受限、无限两种模式的选择和一致更新方式。
- 新增独立的 1Panel 生产更新文档，覆盖备份、标签切换、全服务镜像更新、配置门禁、容器与
  真实设备验收、回滚和脱敏实机记录；明确 Frontend 启动中状态和 Seed 日志密码泄露风险。
...
- 修复系统日志读取审计的重复记录与 metadata 污染；分页查询不再记录无意义的分页参数，
  系统日志读取仍保留可追溯审计。
- 浏览器黑盒通过公开 HTTP API 独立创建并清理两类令牌测试数据，不再依赖前序测试遗留状态。
