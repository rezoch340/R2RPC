...
| 前端浏览器 E2E | ✅ Playwright 10 项，只走 UI + HTTP API，边界守卫禁止直连持久层 |
| 设备 SDK | 不在本仓库范围 |
...
前端 Playwright 10 项冒烟先通过登录页获取真实 JWT，再逐页调用公开 HTTP API，并验证筛选
分页、令牌作用域编辑、复制回退、日志抽屉、导航预取和登录保护。静态边界守卫拒绝 E2E
