...
| `user / role / permission` | 后台 JWT 与 CASL RBAC |
...
当前黑盒套件覆盖所有 HTTP controller 方法，以及 WS 鉴权、welcome、heartbeat、主动 ping、读超时、4 MiB 帧限制、拒绝分片、RPC result 身份匹配/去重、设备 AppAudit 成功/失败 Step 与非法审计隔离、maxInFlight 和在线/离线持久态。
