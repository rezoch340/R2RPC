...
| 黑盒完整性冒烟 | ✅ 180 项，只走 HTTP/WS |
| 后端单元测试 | ✅ Jest 10 suites / 35 tests |
...
当前 180 项黑盒套件覆盖所有 HTTP controller 方法、登录/读取/拒绝/写入系统审计与敏感字段隔离、权限组编辑/
...
RPC 调试、筛选分页、令牌作用域与 Access Token 过期策略编辑、复制回退、日志抽屉、导航预取
和登录保护。静态边界守卫拒绝 E2E
