...
| 黑盒完整性冒烟 | ✅ 162 项，只走 HTTP/WS |
| 代码可读性门禁 | ✅ 禁止含糊缩写；复杂度 ≤ 10、嵌套 ≤ 3、语句 ≤ 40 |
| 管理前端 | ✅ Next.js 16 + shadcn，覆盖全部后台管理公开面 |
| 前端浏览器 E2E | ✅ Playwright 11 项，只走 UI + HTTP API，边界守卫禁止直连持久层 |
| 设备 SDK | 不在本仓库范围 |
...
当前 162 项黑盒套件覆盖所有 HTTP controller 方法、登录/读取/拒绝/写入系统审计与敏感字段隔离、权限组编辑/
...
前端 Playwright 11 项冒烟先通过登录页获取真实 JWT，再逐页调用公开 HTTP API，并验证手动
