...
后台管理 mutation 通过 `@SystemAudit` 记录操作者、动作、对象和结果。系统日志只允许
`read/system-log` 查询，不提供修改或删除入口，也不会保存密码/token 明文。
...
当前 139 项黑盒套件覆盖所有 HTTP controller 方法、系统操作审计与敏感字段隔离、权限组编辑/
