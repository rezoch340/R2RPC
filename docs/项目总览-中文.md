...
> 更新日期：2026-07-25。本文描述当前仓库中的 NestJS 重写版；旧 Go/MySQL 系统文档已移至 `docs/archive/`。

## 1. 项目定位

R2RPC 是设备侧 RPC 中继平台。调用方使用独立 access token，按 `project` 或指定 `clientId` 发起 HTTP 请求；在线设备通过 WebSocket 接收任务、执行并回传结果。

当前仓库包含 NestJS 后端、Next.js 管理前端、Android/Kotlin 与
JavaScript/TypeScript SDK、基础设施编排和文档。具体手机业务 Runtime 由接入方基于 SDK
注册 Action 实现。
...
| 维度 | 状态 |
|---|---|
| 后端与全栈 backlog #1–#24 | ✅ 全部完成 |
| HTTP API | ✅ 39 个路径模板 |
| WebSocket 设备协议 | ✅ 鉴权、心跳、RPC、AppAudit Step、限流及异常帧防护 |
| API / Worker 双进程 | ✅ |
| 数据库迁移 | ✅ 11 个 Drizzle 迁移、15 张表 |
| OpenAPI | ✅ 39 个路径 / 52 个操作，成功与错误响应 schema、双鉴权方案完整 |
| 黑盒完整性冒烟 | ✅ 209 项，只走 HTTP/WS |
| 后端单元测试 | ✅ Jest 11 suites / 40 tests |
| 代码可读性门禁 | ✅ 禁止含糊缩写；复杂度 ≤ 10、嵌套 ≤ 3、语句 ≤ 40 |
| 管理前端 | ✅ Next.js 16 + shadcn，覆盖全部后台管理公开面 |
| 前端浏览器 E2E | ✅ Playwright 13 项，只走 UI + HTTP API，边界守卫禁止直连持久层 |
| Android / JavaScript SDK | ✅ 设备端、调用方、自动重连、超时、AppAudit Recorder |
| SDK 自动化测试 | ✅ JavaScript 11 项、Android 8 项，真实 HTTP/WebSocket Mock Server |
| 生产镜像与编排 | ✅ API/Worker/frontend 镜像 + 完整 Compose |
| 容器性能验收 | ✅ 公开 API 混合压测 + JSON 报告；完整 Compose ≤ 4 核 / 4 GiB |
| 自动化 | ✅ 本地后端/前端质量门禁与 Compose 黑盒；`v*` 标签仅发布 GHCR |
| 公网入口 | ✅ loopback 容器端口 + Nginx/OpenResty + Cloudflare Full (strict)/WSS |
| 项目治理 | ✅ 安全、贡献、行为、许可归属、Issue/PR 模板和发布流程 |
| 公共 SDK 发布 | ⏸ 当前 UNLICENSED，仅允许本地或经批准的私有制品库 |
| 专用健康检查 | 待补 |
...
```mermaid
flowchart LR
  Caller["调用方<br/>Android / JavaScript SDK"] -->|"HTTP + rk_ access token"| API["NestJS API"]
  Admin["后台管理员"] -->|"浏览器"| Frontend["Next.js 控制台"]
  Frontend -->|"HTTP + JWT"| API
  Performance["性能测试容器"] -->|"公开 HTTP API"| API
  Device["设备<br/>Android / JavaScript SDK"] <-->|"WebSocket + dk_ device token"| API
  API --> PG[("PostgreSQL")]
  API --> Redis[("Redis")]
  API --> Queue["BullMQ"]
...
- HTTP、可配置 Swagger 与 `/api/client/ws`
- 做鉴权、校验、RPC 派发及热路径入队
- 默认端口：`3000`
...
- 表格通过公共 `useTableQuery` 每 15 秒自动刷新；切回页面和重置筛选会立即取新，
  后台请求保留当前数据并且不锁定分页控件
...
- API、Worker、迁移、种子和前端共用 `config.yaml` schema；前端只向浏览器注入
  `frontend.apiUrl/apiPort`
- 生产可通过 `app.openApiEnabled: false` 不注册运行时 `/docs`；静态 OpenAPI 继续保留
- 可选 performance 容器同样读取统一配置，挂载虚拟 WS 设备并通过公开接口执行 Hello 混合压测

### 官方 SDK

- `sdk/javascript/`：ESM TypeScript 包，同时提供 `R2RpcDevice`、`R2RpcCaller` 和
  `AppAuditRecorder`
- `sdk/android/`：`minSdk 21`、Java 8 字节码兼容 Android AAR，默认使用 Widevine
  MediaDrm ID 作为设备 `clientId`，网络与并发由 OkHttp、Coroutines 负责
- 两端都内置 welcome 校验、heartbeat、鉴权失败停止、异常断线指数退避、Action 超时与
  标准错误 result
- SDK 测试只走公开协议，不导入后端模块或访问 PostgreSQL、Redis、Manticore
...
| `device token` | `dk_` 前缀；无过期时间，撤销或删除才失效；可二次编辑 project，更新时断开旧作用域连接 |
| `access token` | `rk_` 前缀；调用方 invoke 凭证，可二次编辑 project、绝对过期时间和最大调用次数，累计次数不清零 |
...
2. API 校验 token project 作用域、project enabled 和可选次数上限；受限令牌以 PostgreSQL
   条件更新原子占用一次额度。管理员更新 access token 的作用域或过期策略后会立即清除鉴权
   缓存，下一次调用即使用新配置。
3. Redis 轮询选择在线且未达到 `maxInFlight` 的设备；设备上报的 `1～1024` 整数容量原样采用，
   缺失/非法默认 `16`，服务端只限制超过 `1024` 的异常高值。
...
- `pnpm performance`：固定速率性能测试，只访问运行中的公开 HTTP API。
- `sdk/javascript` 的 `corepack pnpm check`：TypeScript、Vitest 与制品构建。
- `sdk/android` 的
  `./gradlew :r2rpc-sdk:testDebugUnitTest :r2rpc-sdk:assembleRelease`：Kotlin、HTTP/WS
  MockWebServer 测试与 Maven 制品构建。
- `test/assert-blackbox-e2e.js`：阻止 E2E 和性能执行器导入数据库、Redis或应用内部服务。
- `pnpm test:integration:*`：内部算法集成检查，明确不是 E2E。

当前 209 项黑盒套件覆盖所有 HTTP controller 方法、登录/读取/拒绝/写入系统审计与敏感字段隔离、权限组编辑/
嵌套权限/用户分组/root 写隔离、用户资料/改密、管理员写隔离、两类令牌作用域二次编辑和
Access Token 时间/次数策略、并发原子计数及额度用尽 `429`，以及 WS 鉴权、welcome、
...
前端 Playwright 13 项冒烟先通过登录页获取真实 JWT，再逐页调用公开 HTTP API，并验证手动
RPC 调试、筛选分页、令牌作用域与 Access Token 过期策略编辑、复制回退、日志抽屉、慢接口下的
非阻塞切页、表格切回/重置/定时刷新和登录保护。静态边界守卫拒绝 E2E
导入后端内部模块、PG/Drizzle/Redis 客户端或 SQL；因此前端和后端 E2E 口径一致。

性能执行器通过统一配置控制功能组、虚拟设备数、预热、持续时间、并发、目标速率、请求超时
及错误率/P95/吞吐阈值。默认挂 4 台真实 WebSocket 设备，分别压测手动自动路由、
Access Token 自动轮询和随机指定设备 Hello，并要求正式样本覆盖全部设备。受限隔离全栈
基线为 1600 请求、0 失败、80.03 req/s、P95 7.50 ms。全部 Compose 服务 CPU 上限合计
4.00 核，内存上限合计 3840 MiB。

设备接入见 `sdk/README.md`，审计协议见 `docs/device-app-audit.md`。

## 9. 已知非阻塞事项

- API/Worker/frontend 生产镜像、完整 Compose 和受限容器性能验收已完成；仍缺
  CI、Kubernetes/生产 secret、统一反向代理和专用 `/health`、`/ready`。
...
- `sdk/README.md`：Android 与 JavaScript SDK 接入
- `docs/R2RPC-核心功能统计.md`：当前能力矩阵
- `docs/后端进度.md`：唯一进度真源
...
- `docs/superpowers/specs/2026-07-24-unified-configuration-compose-design.md`：统一配置与完整 Compose 设计
- `docs/superpowers/specs/2026-07-24-container-performance-suite-design.md`：容器性能测试与资源预算设计
- `docs/superpowers/specs/2026-07-24-device-sdks-design.md`：Android 与 JavaScript SDK 设计
- `docs/openapi.yaml`：HTTP OpenAPI
- `deploy/README.md`：宿主机开发与完整 Compose
