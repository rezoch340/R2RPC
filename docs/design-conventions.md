...
- 所有实体数据表必须通过公共 `useTableQuery` 查询，统一保留上一份数据、每 15 秒自动刷新，
  并在页面重新进入、窗口重新获得焦点或网络恢复时后台获取最新数据；页面不得重复声明
  `keepPreviousData`、`refetchInterval` 或表格保鲜策略。
- 所有 `FilterBar` 重置操作必须同时清空筛选、回到第一页并调用公共
  `refreshTableData`；后台刷新期间保留已有表格，不显示整表骨架，也不得锁定分页控件。
- 服务端分页只在 query key 切换且正在使用占位数据时锁定分页操作；客户端分页的数据已在
  内存中，不得因后台查询刷新而锁定分页。
