...
当前 Playwright 基线为 **13 passed**，覆盖全部管理页、手动 RPC 调试、字段筛选分页、两类
令牌功能组二次编辑、Access Token 时间/次数策略编辑、非安全上下文复制回退、请求详情抽屉、系统日志长描述列宽隔离、账号改密
入口、移动导航、慢接口下的非阻塞切页、表格切回/重置/定时刷新和登录保护。
...
- 分页请求期间保留上一页数据；侧栏使用标准客户端导航立即切页，不等待业务接口，也不在菜单项
  展示阻塞进度。首次进入且没有缓存时，只在目标页内容区显示尺寸稳定的加载状态。
- 数据表格统一使用 `useTableQuery`：每 15 秒刷新，重新进入页面时立即后台取新，
  轮询期间保留旧数据且不锁定分页；筛选重置必须调用公共 `refreshTableData`。
